...
#include "Turret/CannonTurret.hpp"
...
void PlayScene::ConstructUI() {
    ...
    btn = new TurretButton("play/floor.png", "play/dirt.png",
                           ...
                           Engine::Sprite("play/turret-3.png", 1446, 136 - 8, 0, 0, 0, 0), 1446, 136, CannonTurret::Price);
...
void PlayScene::UIBtnClicked(int id) {
    ...
    else if (id == 2 && money >= CannonTurret::Price)
        preview = new CannonTurret(0, 0);
    if (!preview)
        return;
    preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
    ...
    while (!que.empty()) {
        ...
        for(Engine::Point d : directions){
            d = d+p;
            if(d.x >= 0 && d.x < MapWidth && d.y >= 0 && d.y < MapHeight){
                if(mapState[d.y][d.x] == TILE_DIRT){
                    if(map[d.y][d.x] == -1){
                        map[d.y][d.x] = map[p.y][p.x] + 1;
                        que.push(d);
                    }
                }
            }
